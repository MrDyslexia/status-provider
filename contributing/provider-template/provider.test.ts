...
vi.mock("../src/lib/example-provider.js", () => ({
  queryExampleProviderStatus: vi.fn(),
}));
...
describe("example provider", () => {
  it("returns attempted:false when not configured", async () => {
    const { queryExampleProviderStatus } = await import("../src/lib/example-provider.js");
    (queryExampleProviderStatus as any).mockResolvedValueOnce(null);

    const out = await exampleProviderProvider.fetch({} as any);
    expectNotAttempted(out);
  });

  it("maps success into a status entry", async () => {
    const { queryExampleProviderStatus } = await import("../src/lib/example-provider.js");
    (queryExampleProviderStatus as any).mockResolvedValueOnce({
      success: true,
      percentRemaining: 75,
  ...
  it("maps provider errors into status errors", async () => {
    const { queryExampleProviderStatus } = await import("../src/lib/example-provider.js");
    (queryExampleProviderStatus as any).mockResolvedValueOnce({
      success: false,
      error: "Unauthorized",
