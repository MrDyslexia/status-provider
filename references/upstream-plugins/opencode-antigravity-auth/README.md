...
- **Dual status system** — access both Antigravity and Gemini CLI statuss from one plugin
...
**Antigravity status** (default routing for Claude and Gemini):
...
**Gemini CLI status** (separate from Antigravity; used when `cli_first` is true or as fallback):
...
> **Routing Behavior:**
> - **Antigravity-first (default):** Gemini models use Antigravity status across accounts.
> - **CLI-first (`cli_first: true`):** Gemini models use Gemini CLI status first.
> - When a Gemini status pool is exhausted, the plugin automatically falls back to the other pool.
...
Add multiple Google accounts for higher combined statuss. The plugin automatically rotates between accounts when one is rate-limited.
...
- **Check statuss** — View remaining API status for each account
- **Manage accounts** — Enable/disable specific accounts for rotation

For details on load balancing, dual status pools, and account storage, see [docs/MULTI-ACCOUNT.md](docs/MULTI-ACCOUNT.md).
...
### "All Accounts Rate-Limited" (But Status Available)
...
| Option | Default | What it does |
|--------|---------|--------------|
| `soft_status_threshold_percent` | `90` | Skip account when status usage exceeds this percentage. Prevents Google from penalizing accounts that fully exhaust status. Set to `100` to disable. |
| `status_refresh_interval_minutes` | `15` | Background status refresh interval. After successful API requests, refreshes status cache if older than this interval. Set to `0` to disable. |
| `soft_status_cache_ttl_minutes` | `"auto"` | How long status cache is considered fresh. `"auto"` = max(2 × refresh interval, 10 minutes). Set a number (1-120) for fixed TTL. |

> **How it works**: Status cache is refreshed automatically after API requests (when older than `status_refresh_interval_minutes`) and manually via "Check statuss" in `opencode auth login`. The threshold check uses `soft_status_cache_ttl_minutes` to determine cache freshness - if cache is older, the account is considered "unknown" and allowed (fail-open). When ALL accounts exceed the threshold, the plugin waits for the earliest status reset time (like rate limit behavior). If wait time exceeds `max_rate_limit_wait_seconds`, it errors immediately.
...
- [Multi-Account](docs/MULTI-ACCOUNT.md) — Load balancing, dual status pools, account storage
...
- Respect internal statuss and data handling policies
