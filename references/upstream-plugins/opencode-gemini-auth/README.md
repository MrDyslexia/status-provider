...
you to use your existing Gemini plan and statuss (including the free tier)
...
To check your current Gemini Code Assist status buckets at any time, run:
...
refresh, project/status lookup, and Gemini request forwarding.
...
### Statuss, Plans, and 429 Errors

Common causes of `429 RESOURCE_EXHAUSTED` or `STATUS_EXHAUSTED`:

- **No project ID configured**: the plugin uses a managed free-tier project, which has lower statuss.
- **Model-specific limits**: statuss are tracked per model (e.g., `gemini-3-pro-preview` vs `gemini-3-flash-preview`).
- **Large prompts**: OAuth/Code Assist does not support cached content, so long system prompts and history can burn status quickly.
- **Parallel sessions**: multiple Opencode windows can drain the same bucket.

Notes:

- **Gemini CLI auto-fallbacks**: the official CLI may fall back to Flash when Pro statuss are exhausted, so it can appear to “work” even if the Pro bucket is depleted.
...
endpoints. In particular, project onboarding and status retry handling follow
the same behavior patterns as the Gemini CLI.

### References

- Gemini CLI repository: https://github.com/google-gemini/gemini-cli
- Gemini CLI status documentation: https://developers.google.com/gemini-code-assist/resources/statuss
