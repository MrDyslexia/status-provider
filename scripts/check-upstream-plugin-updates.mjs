...
function getTrackedEntry(lock, pluginId) {
  ...
  if (!tracked) {
    throw new Error(
      `Missing tracked upstream entry for ${pluginId} in references/upstream-plugins/lock.json. Run bun run upstream:sync to rebuild it.`,
    );
  }
  return tracked;
}
...
function createGitHubHeaders(token) {
  return {
    ...
    "User-Agent": "status-provider-upstream-plugin-check",
