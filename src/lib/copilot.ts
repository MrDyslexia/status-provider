 ...
 * `GET /copilot_internal/user` for OAuth-backed personal status checks.
 */
...
const COPILOT_STATUS_CONFIG_FILENAME = "copilot-status-token.json";
const USER_AGENT = "status-provider/copilot-billing";
...
type CopilotStatusApi = "github_billing_api" | "copilot_internal_user" | "none";
type CopilotBillingMode = "user_status" | "organization_usage" | "enterprise_usage" | "none";
...
export interface CopilotPatReadResult {
  ...
  checkedPaths: string[];
  selectedPath?: string;
  config?: CopilotStatusConfig;
  error?: string;
  tokenKind?: CopilotPatTokenKind;
}

export interface CopilotStatusAuthDiagnostics {
...
function resolvePatBillingTarget(config: CopilotStatusConfig): {
  target: CopilotBillingTarget | null;
  error?: string;
} {
  ...
  if (config.tier === "business") {
    if (config.enterprise) {
      return {
        target: null,
        error:
          'Copilot business usage is organization-scoped. Remove "enterprise" and keep "organization" in copilot-status-token.json.',
      };
    }
    ...
    if (!config.organization) {
      return {
        target: null,
        error:
          'Copilot business usage requires an organization-scoped billing report. Add "organization": "your-org-slug" to copilot-status-token.json.',
      };
    }
  ...
  if (config.tier === "enterprise") {
    ...
    return {
      target: null,
      error:
        'Copilot enterprise usage requires an enterprise- or organization-scoped billing report. Add "enterprise": "your-enterprise-slug" or "organization": "your-org-slug" to copilot-status-token.json.',
    };
  }
  ...
  if (config.organization || config.enterprise) {
    return {
      target: null,
      error:
        `Copilot ${config.tier} usage is user-scoped. Remove "organization"/"enterprise" from copilot-status-token.json or switch to a managed tier.`,
    };
  }
...
export function getCopilotPatConfigCandidatePaths(): string[] {
  ...
  return dedupeStrings(
    configDirs.map((configDir) => join(configDir, COPILOT_STATUS_CONFIG_FILENAME)),
  );
}

function validateStatusConfig(raw: unknown): { config: CopilotStatusConfig | null; error?: string } {
...
export function readStatusConfigWithMeta(): CopilotPatReadResult {
  ...
  for (const path of checkedPaths) {
    ...
    try {
      ...
      const validated = validateStatusConfig(parsed);
...
export function getCopilotStatusAuthDiagnostics(authData: AuthData | null): CopilotStatusAuthDiagnostics {
  const pat = readStatusConfigWithMeta();
  ...
  const statusApi: CopilotStatusApi =
    effectiveSource === "pat"
      ? pat.state === "valid"
        ? "github_billing_api"
        : "none"
  ...
  return {
    ...
    billingApiAccessLikely:
      effectiveSource === "pat"
        ...
        : statusApi === "copilot_internal_user",
...
function toUserStatusResultFromCopilotInternal(response: unknown): CopilotStatusResult {
  const totalPaths = [
    ["status", "limit"],
    ["status", "total"],
    ["monthly_status", "limit"],
    ["monthly_status", "total"],
    ["monthly_premium_requests", "limit"],
    ["monthly_premium_requests", "total"],
    ["premium_requests", "limit"],
    ["premium_requests", "total"],
    ["status_snapshots", "premium_interactions", "entitlement"],
    ["limit"],
    ["total"],
    ["status_limit"],
    ["monthly_limit"],
    ["included_premium_requests"],
    // Copilot Free tier: `monthly_statuss.chat` and `monthly_statuss.completions`
    ["monthly_statuss", "chat"],
    ["monthly_statuss", "completions"],
  ];
  const usedPaths = [
    ["status", "used"],
    ["monthly_status", "used"],
  ...
  const remainingPaths = [
    ["status", "remaining"],
    ["monthly_status", "remaining"],
    ["monthly_premium_requests", "remaining"],
    ["premium_requests", "remaining"],
    ["status_snapshots", "premium_interactions", "remaining"],
    ["status_snapshots", "premium_interactions", "status_remaining"],
    ["remaining"],
    ["status_remaining"],
    ["monthly_remaining"],
    ["premium_requests_remaining"],
    // Copilot Free tier: `limited_user_statuss` tracks remaining
    ["limited_user_statuss", "chat"],
    ["limited_user_statuss", "completions"],
  ];
  const resetPaths = [
    ["status", "reset_at"],
    ["monthly_status", "reset_at"],
    ["monthly_premium_requests", "reset_at"],
    ["premium_requests", "reset_at"],
    ["reset_at"],
    ["status_reset_date_utc"],
    ["status_reset_date"],
    ["status_reset_at"],
    // Copilot Free tier
  ...
  const percentRemainingPaths = [
    ["status", "percent_remaining"],
    ["monthly_status", "percent_remaining"],
    ["monthly_premium_requests", "percent_remaining"],
    ["premium_requests", "percent_remaining"],
    ["status_snapshots", "premium_interactions", "percent_remaining"],
    ["percent_remaining"],
  ];
  const unlimitedPaths = [
    ["status", "unlimited"],
    ["monthly_status", "unlimited"],
    ["monthly_premium_requests", "unlimited"],
    ["premium_requests", "unlimited"],
    ["status_snapshots", "premium_interactions", "unlimited"],
    ["unlimited"],
  ];
  const tierPaths = [
    ["plan", "type"],
  ...
  if (!Number.isFinite(total) || total === undefined || total <= 0 || used === undefined || used < 0) {
    throw new Error(
      "GitHub /copilot_internal/user response did not include usable personal status fields.",
    );
  }
...
function toUserStatusResultFromBilling(
  response: BillingUsageResponse,
  ...
  if (!total || total <= 0) {
    throw new Error(
      "Copilot billing response did not include a limit. Configure copilot-status-token.json with your tier so the plugin can compute status totals.",
    );
  }
...
function toStatusError(message: string): StatusError {
 ...
 */
export async function queryCopilotStatus(options: { requestTimeoutMs?: number } = {}): Promise<CopilotResult> {
  const pat = readStatusConfigWithMeta();

  if (pat.state === "invalid") {
    return toStatusError(
      `Invalid copilot-status-token.json: ${pat.error ?? "unknown error"}${pat.selectedPath ? ` (${pat.selectedPath})` : ""}`,
    );
  }
  ...
  if (pat.state === "valid" && pat.config) {
    ...
    if (!resolvedTarget.target) {
      return toStatusError(resolvedTarget.error ?? "Unable to resolve Copilot billing scope.");
    }
    ...
    if (tokenCompatibilityError) {
      return toStatusError(tokenCompatibilityError);
    }
    ...
    try {
      ...
      return toUserStatusResultFromBilling(response, pat.config.tier);
    } catch (error) {
      return toStatusError(error instanceof Error ? error.message : String(error));
    }
  }
  ...
  try {
    ...
    return toUserStatusResultFromCopilotInternal(response);
  } catch (error) {
    return toStatusError(error instanceof Error ? error.message : String(error));
  }
}

export async function hasCopilotStatusRuntimeAvailable(): Promise<boolean> {
  const diagnostics = getCopilotStatusAuthDiagnostics(await readAuthFile());
  return diagnostics.billingApiAccessLikely;
}

export function formatCopilotStatus(result: CopilotResult): string | null {
