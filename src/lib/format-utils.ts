/**
 * Shared formatting utilities for status display.
 *
 * These primitives are used by:
 * - format.ts (classic toast)
 * - toast-format-grouped.ts (grouped toast)
 * - status-command-format.ts (/status-provider command)
 */
...
// Matches CSI sequences like ESC[...m and other common ANSI escape codes.
...
/**
 * Pad a string to `width` visible characters by appending spaces.
 * Handles strings that may contain ANSI color codes.
 */
export function padRightVisible(str: string, width: number): string {
...
/**
 * Wrap an array of lines in box-drawing borders.
 *
 * Each line is padded to `innerWidth` visible characters, then surrounded with
 * "│ " on the left and " │" on the right.  A top and bottom border are added.
 * The total width of every output line is `innerWidth + 4`.
 */
export function boxWrapLines(lines: string[], innerWidth: number): string[] {
  const total = innerWidth + 4;
  const horizontal = "─".repeat(total - 2);
  return [
    `┌${horizontal}┐`,
    ...lines.map((line) => `│ ${padRightVisible(line, innerWidth)} │`),
    `└${horizontal}┘`,
  ];
}
