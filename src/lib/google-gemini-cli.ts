...
const GEMINI_STATUS_API_URL = "https://cloudcode-pa.googleapis.com/v1internal:retrieveUserStatus";
const GEMINI_TOKEN_TIMEOUT_MS = 8_000;
const GEMINI_STATUS_TIMEOUT_MS = 6_000;
const GEMINI_ACCOUNTS_CONCURRENCY = 3;
const GEMINI_CLI_USER_AGENT = `GeminiCLI/status-provider (${process.platform}; ${process.arch})`;
...
type RetrieveUserStatusResponse = {
  buckets?: RetrieveUserStatusBucket[];
};
...
function resolveProjectId(
  ...
  return (
    ...
    normalizeString(entry.statusProjectId) ??
    parts.projectId ??
...
export async function hasGeminiCliStatusRuntimeAvailable(client?: ConfigClient): Promise<boolean> {
...
async function retrieveGeminiCliStatus(
  accessToken: string,
  projectId: string,
  timeoutMs: number = GEMINI_STATUS_TIMEOUT_MS,
): Promise<RetrieveUserStatusResponse> {
  const response = await fetchWithTimeout(
    GEMINI_STATUS_API_URL,
    {
      method: "POST",
      headers: {
  ...
  if (!response.ok) {
    if (response.status === 401 || response.status === 403) {
      throw new Error(`Gemini CLI status auth error: ${response.status}`);
    }
    throw new Error(`Gemini CLI status API error: ${response.status}`);
  }

  return response.json() as Promise<RetrieveUserStatusResponse>;
}
...
function aggregateGeminiCliQualityTiers(
  ...
  const groupedBuckets = new Map<GeminiCliQualityTierKey, GeminiCliStatusBucket>();
  const unknownBuckets: GeminiCliStatusBucket[] = [];
...
function mapStatusBuckets(
  buckets: RetrieveUserStatusBucket[] | undefined,
  account: GeminiCliAccount,
): GeminiCliStatusBucket[] {
  if (!buckets) {
    return [];
...
async function fetchAccountStatus(params: {
  account: GeminiCliAccount;
  credentials: GeminiCliConfiguredCredentials;
  timeoutMs?: number;
}): Promise<{
  ...
  try {
    ...
    let status: RetrieveUserStatusResponse;
    try {
      status = await retrieveGeminiCliStatus(
    ...
    return {
      success: true,
      buckets: mapStatusBuckets(status.buckets, params.account),
      accountEmail,
    };
  } catch (err) {
    if (err instanceof Error && err.message.includes("timeout")) {
...
export async function queryGeminiCliStatus(
  client?: ConfigClient,
  ...
  const results = await mapWithConcurrency({
    ...
    fn: async (account) =>
      fetchAccountStatus({ account, credentials, timeoutMs: options.requestTimeoutMs }),
  });

  const allBuckets: GeminiCliStatusBucket[] = [];
  ...
  if (allBuckets.length === 0 && errors.length === 0) {
    return {
      success: false,
      error: "No Gemini CLI status data available",
    };
  }
