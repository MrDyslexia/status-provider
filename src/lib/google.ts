/**
 * Google Antigravity status fetcher
 *
...
//   legacy key "opencode-qwencode-auth", and uses local status state.
...
const GOOGLE_STATUS_API_URL = "https://cloudcode-pa.googleapis.com/v1internal:fetchAvailableModels";
...
const GOOGLE_STATUS_TIMEOUT_MS = 6000;

// Multi-account fetching concurrency (reliability > speed).
const GOOGLE_ACCOUNTS_CONCURRENCY = 3;

function getCompanionStatusError(state: "missing" | "invalid"): string {
...
export async function hasAntigravityStatusRuntimeAvailable(): Promise<boolean> {
...
export async function refreshGoogleTokensForAllAccounts(params?: {
  ...
  if (credentials.state !== "configured") {
    const error = getCompanionStatusError(credentials.state);
    return {
      total: valid.length,
      successCount: 0,
...
/**
 * Fetch status from Google API
 */
async function fetchGoogleStatus(
  accessToken: string,
  projectId: string,
  timeoutMs: number = GOOGLE_STATUS_TIMEOUT_MS,
): Promise<GoogleStatusResponse> {
  const response = await fetchWithTimeout(
    GOOGLE_STATUS_API_URL,
    {
      method: "POST",
      headers: {
...
/**
 * Extract model statuss from API response
 */
function extractModelStatuss(
  data: GoogleStatusResponse,
  modelIds: GoogleModelId[],
  accountEmail?: string,
): GoogleModelStatus[] {
  const statuss: GoogleModelStatus[] = [];
  ...
  for (const modelId of modelIds) {
    ...
    if (modelInfo) {
      const remainingFraction = modelInfo.statusInfo?.remainingFraction ?? 0;
      statuss.push({
        modelId,
        displayName: modelConfig.display,
        percentRemaining: Math.round(remainingFraction * 100),
        resetTimeIso: modelInfo.statusInfo?.resetTime,
        accountEmail,
      });
    }
  }
...
/**
 * Fetch status for a single account
 */
...
async function fetchAccountStatusWithAntigravityRefresh(params: {
  ...
  try {
    ...
    let data: GoogleStatusResponse;
    try {
      data = await fetchGoogleStatus(tokenResult.accessToken, projectId, params.timeoutMs);
    } catch (err) {
      // One auth retry: refresh token then retry status call.
      if (err instanceof Error && err.message.includes("auth error")) {
        ...
        data = await fetchGoogleStatus(retryToken.accessToken, projectId, params.timeoutMs);
      } else {
        throw err;
      }
    }
    const models = extractModelStatuss(data, params.modelIds, email);
...
/**
 * Query Google Antigravity status for ALL accounts
 *
 * Reads accounts from ~/.config/opencode/antigravity-accounts.json.
 * Refreshes access tokens and fetches status for all accounts in parallel.
 *
 * @param modelIds - Model IDs to fetch status for
 * @returns Status result with all models and any errors, or null if not configured
 */
export async function queryGoogleStatus(
  modelIds: GoogleModelId[],
  ...
  if (credentials.state !== "configured") {
    return {
      success: false,
      error: getCompanionStatusError(credentials.state),
    };
  }
  ...
  const results = await mapWithConcurrency({
    ...
    concurrency: GOOGLE_ACCOUNTS_CONCURRENCY,
    fn: async (account) =>
      fetchAccountStatusWithAntigravityRefresh({
        account,
        modelIds,
        credentials,
  ...
  const allModels: GoogleModelStatus[] = [];
  ...
  } as GoogleStatusResult;
}

/**
 * Format Google status for toast display
 *
 * @param result - Google status result
 * @returns Formatted string like "G3Pro 100% * G3Flash 100% * Claude 0%" or null
 */
export function formatGoogleStatus(result: GoogleResult): string | null {
  if (!result) {
    return null;
  }
