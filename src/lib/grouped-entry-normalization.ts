...
export type GroupedRenderTarget = "toast" | "status";

export type NormalizedGroupedStatusEntry = StatusProviderEntry & {
  group: string;
};

export type StatusEntryGroup = {
  group: string;
  entries: NormalizedGroupedStatusEntry[];
};

type RankedGroupedStatusEntry = {
  entry: NormalizedGroupedStatusEntry;
...
function normalizeGroupedStatusEntry(
  ...
  if (googleFallback) {
    return {
      ...
      ...(label || target === "status" ? { label: label ?? googleFallback.label } : {}),
    };
  }
  ...
  return {
    ...
    ...(target === "status" ? { label: label ?? "Status:" } : {}),
  };
}
...
export function groupStatusEntries(
  ...
  const groupedEntries = new Map<string, RankedGroupedStatusEntry[]>();

  for (const [originalIndex, entry] of entries.entries()) {
    const normalizedEntry = normalizeGroupedStatusEntry(entry, target);
    const rankedEntry: RankedGroupedStatusEntry = {
...
export function normalizeGroupedStatusEntries(
  entries: StatusProviderEntry[],
  target: GroupedRenderTarget,
): NormalizedGroupedStatusEntry[] {
  return groupStatusEntries(entries, target).flatMap((group) => group.entries);
}
