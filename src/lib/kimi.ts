import type { KimiResult, KimiStatusWindow, StatusError } from "./types.js";
...
function parseKimiUsagePayload(payload: Record<string, unknown>): {
  ...
  const windows: KimiStatusWindow[] = [];
...
type FetchResult =
  | { ok: true; windows: KimiStatusWindow[]; topLevelKeys: string[] }
  | { ok: false; error: string };

async function fetchKimiStatusFromUrl(
  url: string,
  apiKey: string,
...
export async function queryKimiStatus(options: { requestTimeoutMs?: number } = {}): Promise<KimiResult> {
  ...
  const result = await fetchKimiStatusFromUrl(KIMI_USAGE_URL, auth.apiKey, options.requestTimeoutMs);
