import type { CanonicalStatusProviderId } from "./provider-metadata.js";
import { STATUS_PROVIDER_RUNTIME_IDS } from "./provider-metadata.js";
...
export function modelProviderMatchesRuntimeId(
  model: string,
  canonicalId: CanonicalStatusProviderId,
): boolean {
  const { providerId } = parseProviderModelRef(model);
  return STATUS_PROVIDER_RUNTIME_IDS[canonicalId].includes(providerId);
}
