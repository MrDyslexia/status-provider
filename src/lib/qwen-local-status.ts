...
const QWEN_LOCAL_STATUS_STATE_VERSION = 1 as const;
...
export interface QwenLocalStatusStateFileV1 {
  version: 1;
  utcDay: string;
  dayCount: number;
  recent: number[];
...
export interface QwenComputedStatus {
  day: {
    used: number;
    limit: number;
...
interface RollingComputedStatusWindow {
  used: number;
  limit: number;
  percentRemaining: number;
  resetTimeIso?: string;
...
export interface AlibabaCodingPlanComputedStatus {
  tier: AlibabaCodingPlanTier;
  fiveHour: RollingComputedStatusWindow;
  weekly: RollingComputedStatusWindow;
  monthly: RollingComputedStatusWindow;
}
...
function defaultQwenState(nowMs: number): QwenLocalStatusStateFileV1 {
  return {
    version: QWEN_LOCAL_STATUS_STATE_VERSION,
...
function normalizeQwenState(raw: unknown, nowMs: number): QwenLocalStatusStateFileV1 {
  if (!raw || typeof raw !== "object") {
    return defaultQwenState(nowMs);
  }

  const obj = raw as Partial<QwenLocalStatusStateFileV1>;
...
function applyUtcResetAndPrune(state: QwenLocalStatusStateFileV1, nowMs: number): QwenLocalStatusStateFileV1 {
...
export function getQwenLocalStatusPath(): string {
  const { stateDir } = getOpencodeRuntimeDirs();
  return join(stateDir, "status-provider", "qwen-local-status.json");
}

export function getAlibabaCodingPlanStatusPath(): string {
  const { stateDir } = getOpencodeRuntimeDirs();
  return join(stateDir, "status-provider", "alibaba-coding-plan-local-status.json");
}

export async function readQwenLocalStatusState(params?: { nowMs?: number }): Promise<QwenLocalStatusStateFileV1> {
  const nowMs = params?.nowMs ?? Date.now();
  const path = getQwenLocalStatusPath();
...
export async function readAlibabaCodingPlanStatusState(params?: {
  nowMs?: number;
}): Promise<AlibabaCodingPlanStateFileV1> {
  const nowMs = params?.nowMs ?? Date.now();
  const path = getAlibabaCodingPlanStatusPath();
...
export async function recordQwenCompletion(params?: { atMs?: number }): Promise<QwenLocalStatusStateFileV1> {
  ...
  const next: QwenLocalStatusStateFileV1 = {
    ...state,
...
export function computeQwenStatus(params: {
  state: QwenLocalStatusStateFileV1;
  nowMs?: number;
  dayLimit?: number;
  rpmLimit?: number;
}): QwenComputedStatus {
