import type { AggregateResult, SessionTreeNode, TokenBuckets } from "./status-stats.js";
...
type StatusStatsReportKind = "standard" | "session" | "session_tree";
...
export function formatStatusStatsReport(params: {
  title: string;
  ...
  sessionOnly?: boolean;
  reportKind?: StatusStatsReportKind;
  sessionTree?: {
  ...
  if (sessionTreeMode && !sessionTree) {
    throw new Error("formatStatusStatsReport requires sessionTree for session_tree reports");
  }
  ...
  if (r.unknown.length > 0) {
    sections.push({
      ...
      blocks: [
        ...
        {
          kind: "lines",
          lines: ["Run /status-provider-info to see the full pricing diagnostics report."],
        },
      ],
    });
  }
