/**
 * Synthetic status fetcher
 *
 * Resolves API key from multiple sources and queries:
 * https://api.synthetic.new/v2/statuss
 */

import type { StatusError, SyntheticResult, SyntheticStatusWindow } from "./types.js";
...
const SYNTHETIC_STATUS_URL = "https://api.synthetic.new/v2/statuss";
...
function invalidSyntheticResponse(message: string): StatusError {
  return {
    success: false,
    error: message,
  };
...
function buildRollingFiveHourWindow(payload: Record<string, unknown>): SyntheticStatusWindow | StatusError {
  const rolling = asRecord(payload.rollingFiveHourLimit);
  if (!rolling) {
    return invalidSyntheticResponse("Synthetic API response missing rollingFiveHourLimit status window");
  }
...
function buildWeeklyTokenWindow(payload: Record<string, unknown>): SyntheticStatusWindow | StatusError {
  const weekly = asRecord(payload.weeklyTokenLimit);
  if (!weekly) {
    return invalidSyntheticResponse("Synthetic API response missing weeklyTokenLimit status window");
  }
...
export async function querySyntheticStatus(options: { requestTimeoutMs?: number } = {}): Promise<SyntheticResult> {
  const resolved = await resolveSyntheticApiKey();
  if (!resolved) return null;

  try {
    const resp = await fetchWithTimeout(
      SYNTHETIC_STATUS_URL,
      {
        method: "GET",
        headers: {
