...
function formatCompactPercentLabel(
  ...
  mode: StatusProviderConfig["percentDisplayMode"],
): string {
...
function getProviderName(entry: StatusProviderEntry): string {
...
function getWindowLabel(entry: StatusProviderEntry): string | null {
  const windowLabel =
...
function formatCompactEntrySegments(params: {
  entries: StatusRenderData["entries"];
...
function formatCompactSessionTokensSegment(data: StatusRenderData): string | null {
...
function formatFirstErrorSegment(errors: StatusProviderError[]): string | null {
...
export function buildCompactStatusStatusLine(params: {
