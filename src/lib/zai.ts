 ...
 * https://api.z.ai/api/monitor/usage/status/limit
 */
...
const ZAI_STATUS_URL = "https://api.z.ai/api/monitor/usage/status/limit";

export async function queryZaiStatus(options: { requestTimeoutMs?: number } = {}): Promise<ZaiResult> {
  ...
  try {
    ...
    const resp = await fetchWithTimeout(ZAI_STATUS_URL, { headers }, options.requestTimeoutMs);
    if (!resp.ok) {
