/**
 * Zhipu status fetcher
 *
 * Uses OpenCode's auth.json (zhipu-coding-plan) and queries:
 * https://bigmodel.cn/api/monitor/usage/status/limit
 */
...
const ZHIPU_STATUS_URL = "https://bigmodel.cn/api/monitor/usage/status/limit";

export async function queryZhipuStatus(options: { requestTimeoutMs?: number } = {}): Promise<ZaiResult> {
  ...
  try {
    ...
    const resp = await fetchWithTimeout(ZHIPU_STATUS_URL, { headers }, options.requestTimeoutMs);
    if (!resp.ok) {
    ...
    const data = (await resp.json()) as ZaiStatusResponse;
