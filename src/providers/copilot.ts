 ...
 *
 * Normalizes Copilot status into generic toast entries.
 */

import type { StatusProvider, StatusProviderContext, StatusProviderResult } from "../lib/entries.js";
import { hasCopilotStatusRuntimeAvailable, queryCopilotStatus } from "../lib/copilot.js";
...
function getCopilotGroup(mode: "user_status" | "organization_usage" | "enterprise_usage"): string {
  return mode === "user_status" ? "Copilot (personal)" : "Copilot (business)";
}
...
export const copilotProvider: StatusProvider = {
  id: "copilot",
  ...
  async isAvailable(ctx: StatusProviderContext): Promise<boolean> {
    ...
    try {
      return await hasCopilotStatusRuntimeAvailable();
    } catch {
      return false;
    }
  },
  ...
  async fetch(ctx: StatusProviderContext): Promise<StatusProviderResult> {
    const result = await queryCopilotStatus({ requestTimeoutMs: ctx.config?.requestTimeoutMs });
