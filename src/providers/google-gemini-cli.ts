...
import { hasGeminiCliStatusRuntimeAvailable, queryGeminiCliStatus } from "../lib/google-gemini-cli.js";
...
async function isGeminiCliConfigured(ctx: StatusProviderContext): Promise<boolean> {
  try {
    return await hasGeminiCliStatusRuntimeAvailable(ctx.client);
  } catch {
    return false;
  }
}

export const googleGeminiCliProvider: StatusProvider = {
  ...
  async fetch(ctx: StatusProviderContext): Promise<StatusProviderResult> {
    const result = await queryGeminiCliStatus(ctx.client, {
