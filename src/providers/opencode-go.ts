 ...
 * weekly, and monthly usage as percentage-based status entries.
 */
...
import { queryOpenCodeGoStatus } from "../lib/opencode-go.js";
...
export const opencodeGoProvider: StatusProvider = {
  id: "opencode-go",
  ...
  matchesCurrentModel(model: string): boolean {
    ...
    return normalizeStatusProviderId(provider) === "opencode-go";
  },
  ...
  async fetch(ctx: StatusProviderContext): Promise<StatusProviderResult> {
    ...
    const result = await queryOpenCodeGoStatus(config.config.workspaceId, config.config.authCookie, {
