...
describe("status-provider bin", () => {
  ...
  it("dispatches show to the status CLI command", async () => {
    const { main } = await import("../src/bin/status-provider.js");
  ...
  it("passes show provider args through to the status CLI command", async () => {
    const { main } = await import("../src/bin/status-provider.js");
  ...
  it("prints help and exits zero for --help", async () => {
    ...
    expect(log).toHaveBeenCalledWith(expect.stringContaining("status-provider show"));
    log.mockRestore();
  });
