...
describe("loadConfig layered precedence", () => {
  ...
  beforeEach(() => {
    ...
    tempDir = mkdtempSync(join(tmpdir(), "status-provider-config-"));
  ...
  it("supports file loading with an explicit cwd override and records layered provenance", async () => {
    ...
    expect(meta.workspaceConfigPaths).toEqual([statusConfigSource(altWorkspaceDir)]);
    expect(meta.paths).toEqual(meta.workspaceConfigPaths);
    expect(meta.settingSources).toEqual({
      enabledProviders: statusConfigSource(altWorkspaceDir),
      formatStyle: statusConfigSource(altWorkspaceDir),
      percentDisplayMode: statusConfigSource(altWorkspaceDir),
      onlyCurrentModel: statusConfigSource(altWorkspaceDir),
    });
  ...
  it("merges pricingSnapshot per field so global and workspace sources can coexist", async () => {
    ...
    expect(meta.settingSources["pricingSnapshot.source"]).toBe(
      statusConfigSource(join(xdgConfigHome, "opencode")),
    );
  ...
  it("merges tuiCompactStatus per field and ignores invalid workspace fields", async () => {
    ...
    expect(meta.settingSources["tuiCompactStatus.suppressWhenNativeProviderStatus"]).toBe(
      statusConfigSource(workspaceDir),
    );
  });
  ...
  it("preserves the previous valid layer when workspace values are invalid", async () => {
    ...
    const globalStatusConfigSource = statusConfigSource(join(xdgConfigHome, "opencode"));
    expect(meta.settingSources.enabledProviders).toBe(
      globalStatusConfigSource,
    );
