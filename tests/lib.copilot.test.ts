...
const patPath = "/home/test/.config/opencode/copilot-status-token.json";
const realEnv = process.env;

describe("queryCopilotStatus", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.restoreAllMocks();
    vi.useFakeTimers();
  ...
  it("returns null when no PAT config and no OpenCode Copilot auth exist", async () => {
    ...
    await expect(queryCopilotStatus()).resolves.toBeNull();
  });
  ...
  it("prefers PAT billing config over OpenCode auth when both exist", async () => {
    ...
    const { queryCopilotStatus } = await import("../src/lib/copilot.js");
    const result = await queryCopilotStatus();
  ...
  it("allows negative percentRemaining when /copilot_internal/user reports usage above the status total", async () => {
    authMocks.readAuthFile.mockResolvedValueOnce({
      "github-copilot-chat": { type: "oauth", access: "oauth_access_token", refresh: "refresh" },
    });
  ...
  it("parses premium_interactions from /copilot_internal/user status_snapshots", async () => {
    authMocks.readAuthFile.mockResolvedValueOnce({
    ...
    const fetchMock = vi.fn(async (url: unknown, init?: RequestInit) => {
      ...
      if (target === "https://api.github.com/copilot_internal/user") {
        ...
        return new Response(
          JSON.stringify({
            login: "slkiser",
            access_type_sku: "free_educational_status",
            copilot_plan: "individual",
            status_reset_date: "2026-04-01",
            status_reset_date_utc: "2026-04-01T00:00:00.000Z",
            status_snapshots: {
              premium_interactions: {
                entitlement: 300,
                status_remaining: 230,
                remaining: 230,
                unlimited: false,
              },
            },
  ...
  it("uses explicit percent_remaining from /copilot_internal/user when present", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async () =>
        new Response(
          JSON.stringify({
            status_reset_date_utc: "2026-05-01T00:00:00.000Z",
            status_snapshots: {
              premium_interactions: {
                entitlement: 1500,
                remaining: 401,
  ...
  it("preserves explicit negative percent_remaining for over-status responses", async () => {
    authMocks.readAuthFile.mockResolvedValueOnce({
      "github-copilot": { type: "oauth", access: "oauth_access_token", refresh: "refresh" },
    });
  ...
  it("treats explicit unlimited premium_interactions as unlimited", async () => {
    ...
    const { formatCopilotStatus, queryCopilotStatus } = await import("../src/lib/copilot.js");
    const result = await queryCopilotStatus();
    ...
    expect(formatCopilotStatus(result)).toBe("Copilot Unlimited");
  });
  ...
  it("does not fall back to OpenCode auth when PAT config is invalid", async () => {
    ...
    expect(result && !result.success ? result.error : "").toContain(
      "Invalid copilot-status-token.json",
    );
  ...
  it("surfaces PAT precedence and organization details in diagnostics", async () => {
    ...
    const { getCopilotStatusAuthDiagnostics } = await import("../src/lib/copilot.js");
    const diagnostics = getCopilotStatusAuthDiagnostics({
      "github-copilot": { type: "oauth", access: "oauth_access_token" },
    });
    ...
    expect(diagnostics.statusApi).toBe("github_billing_api");
  ...
  it("treats an invalid PAT as blocking even when OAuth auth is configured", async () => {
    ...
    expect(diagnostics.effectiveSource).toBe("pat");
    expect(diagnostics.override).toBe("pat_overrides_oauth");
    expect(diagnostics.statusApi).toBe("none");
  ...
  it("surfaces enterprise billing scope and compatibility errors in diagnostics", async () => {
    ...
    const { getCopilotStatusAuthDiagnostics } = await import("../src/lib/copilot.js");
    const diagnostics = getCopilotStatusAuthDiagnostics(null);
