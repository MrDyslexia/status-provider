...
import { queryCrofStatus } from "../src/lib/crof.js";

describe("queryCrofStatus", () => {
  const originalEnv = process.env;
  const originalCwd = process.cwd();
  let tempDir: string;

  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), "status-provider-crof-"));
  ...
  it("returns null when not configured", async () => {
    ...
    await expect(queryCrofStatus()).resolves.toBeNull();
  });
  ...
  it("returns usage data from API", async () => {
    ...
    const out = await queryCrofStatus();
    expect(out).toEqual({
      success: true,
      credits: 12.3456,
      requestsPlan: 500,
