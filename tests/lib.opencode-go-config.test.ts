...
function getConfigPath(configDir: string): string {
  return join(configDir, "status-provider", "opencode-go.json");
}
...
async function createConfigDirs(): Promise<[string, string]> {
  ...
  await mkdir(join(primaryDir, "status-provider"), { recursive: true });
  await mkdir(join(fallbackDir, "status-provider"), { recursive: true });
