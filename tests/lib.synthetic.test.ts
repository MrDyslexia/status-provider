...
import { querySyntheticStatus } from "../src/lib/synthetic.js";

vi.mock("../src/lib/opencode-auth.js", () => ({
  readAuthFile: vi.fn(),
}));

describe("querySyntheticStatus", () => {
  const originalEnv = process.env;
  const originalCwd = process.cwd();
  let tempDir: string;

  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), "status-provider-synthetic-"));
  ...
  it("returns null when not configured", async () => {
    ...
    await expect(querySyntheticStatus()).resolves.toBeNull();
  });

  it("returns both top-level Synthetic status windows from the API", async () => {
    ...
    const out = await querySyntheticStatus();
    expect(out).toEqual({
      success: true,
      windows: {
        fiveHour: {
  ...
  it.each(["bad-value", -5, 150] as const)(
    ...
    async (percentRemaining) => {
      ...
      await expect(querySyntheticStatus()).resolves.toEqual({
        success: true,
        windows: {
          fiveHour: {
            limit: 100,
  ...
  it("rejects malformed top-level rollingFiveHourLimit payloads", async () => {
    ...
    await expect(querySyntheticStatus()).resolves.toEqual({
      success: false,
      error: "Synthetic API response missing rollingFiveHourLimit status window",
    });
  });
  ...
  it("rejects malformed weeklyTokenLimit credit strings", async () => {
    ...
    await expect(querySyntheticStatus()).resolves.toEqual({
      success: false,
      error: "Synthetic API response missing weeklyTokenLimit status window",
    });
  });
