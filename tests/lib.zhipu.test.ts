...
import { queryZhipuStatus } from "../src/lib/zhipu.js";
...
describe("queryZhipuStatus", () => {
  it("returns null when not configured", async () => {
    mocks.resolveZhipuAuthCached.mockResolvedValueOnce({ state: "none" });

    await expect(queryZhipuStatus()).resolves.toBeNull();
  });
  ...
  it("returns auth errors when zhipu-coding-plan auth is invalid", async () => {
    ...
    await expect(queryZhipuStatus()).resolves.toEqual({
      success: false,
      error: 'Unsupported Zhipu auth type: "oauth"',
    });
  });
  ...
  it("returns API status errors with truncated response body", async () => {
    ...
    const out = await queryZhipuStatus();
    expect(out && !out.success ? out.error : "").toBe(
      `Zhipu API error 500: ${body.slice(0, 120)}`,
    );
  });

  it("returns invalid status data when limits is missing or not an array", async () => {
    await mockZhipuAuth();
    vi.stubGlobal(
      "fetch",
      vi.fn(
    ...
    const out = await queryZhipuStatus();
    expect(out).toEqual({ success: false, error: "Invalid status data" });
  });
