...
vi.mock("../src/lib/qwen-local-status.js", () => ({
  ...
  readQwenLocalStatusState: vi.fn(),
  computeQwenStatus: vi.fn(),
  getQwenLocalStatusPath: vi.fn(() => "/tmp/qwen-local-status.json"),
  getAlibabaCodingPlanStatusPath: vi.fn(() => "/tmp/alibaba-local-status.json"),
...
describe("plugin qwen question hook", () => {
  ...
  it("records qwen free completion on successful qwen question execution", async () => {
    ...
    const hooks = await StatusProviderPlugin({
