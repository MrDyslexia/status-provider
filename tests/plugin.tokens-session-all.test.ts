...
vi.mock("../src/lib/status-stats-format.js", () => ({
  formatStatusStatsReport: mocks.formatStatusStatsReport,
}));
...
describe("/tokens_session_all command", () => {
  beforeEach(() => {
    ...
    mocks.formatStatusStatsReport.mockReturnValue("formatted token report");
  ...
  it("aggregates the current session tree for /tokens_session_all", async () => {
    ...
    expect(mocks.formatStatusStatsReport).toHaveBeenCalledWith(
