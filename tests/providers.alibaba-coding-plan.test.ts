...
vi.mock("../src/lib/qwen-local-status.js", () => ({
  readAlibabaCodingPlanStatusState: vi.fn(),
  computeAlibabaCodingPlanStatus: vi.fn(),
}));
...
describe("alibaba-coding-plan provider", () => {
  ...
  it("uses env-based fallback auth with the configured fallback tier", async () => {
    ...
    const { computeAlibabaCodingPlanStatus, readAlibabaCodingPlanStatusState } = await import(
      "../src/lib/qwen-local-status.js"
    );

    (readAlibabaCodingPlanStatusState as any).mockResolvedValue({});
    (computeAlibabaCodingPlanStatus as any).mockReturnValue({
      tier: "pro",
    ...
    expect(computeAlibabaCodingPlanStatus as any).toHaveBeenCalledWith({ state: {}, tier: "pro" });
  });
