...
describe("chutes provider", () => {
  it("returns attempted:false when not configured", async () => {
    const { queryChutesStatus } = await import("../src/lib/chutes.js");
    (queryChutesStatus as any).mockResolvedValueOnce(null);
  ...
  it("maps success into a toast entry", async () => {
    const { queryChutesStatus } = await import("../src/lib/chutes.js");
    (queryChutesStatus as any).mockResolvedValueOnce({
      success: true,
      percentRemaining: 75,
