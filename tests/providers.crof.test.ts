...
describe("crof provider", () => {
  it("returns attempted:false when not configured", async () => {
    const { queryCrofStatus } = await import("../src/lib/crof.js");
    (queryCrofStatus as any).mockResolvedValueOnce(null);
  ...
  it("maps success into request and credit rows", async () => {
    const { queryCrofStatus } = await import("../src/lib/crof.js");
    (queryCrofStatus as any).mockResolvedValueOnce({
      success: true,
      credits: -1.25,
      requestsPlan: 500,
      usableRequests: 469,
