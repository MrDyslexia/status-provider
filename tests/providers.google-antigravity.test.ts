...
vi.mock("../src/lib/google.js", () => ({
  hasAntigravityStatusRuntimeAvailable: vi.fn(),
  queryGoogleStatus: vi.fn(),
}));

describe("google antigravity provider", () => {
  it("returns attempted:false when antigravity accounts are not configured", async () => {
    const { queryGoogleStatus } = await import("../src/lib/google.js");
    (queryGoogleStatus as any).mockResolvedValueOnce(null);
  ...
  it("maps success into toast entries and truncated error labels", async () => {
    const { queryGoogleStatus } = await import("../src/lib/google.js");
    (queryGoogleStatus as any).mockResolvedValueOnce({
      success: true,
      models: [
        {
  ...
  it("is available only when the antigravity runtime is configured", async () => {
    const { hasAntigravityStatusRuntimeAvailable } = await import("../src/lib/google.js");
    (hasAntigravityStatusRuntimeAvailable as any).mockResolvedValueOnce(true);
    await expect(googleAntigravityProvider.isAvailable({} as any)).resolves.toBe(true);

    (hasAntigravityStatusRuntimeAvailable as any).mockResolvedValueOnce(false);
  ...
  it("returns false when runtime detection throws", async () => {
    const { hasAntigravityStatusRuntimeAvailable } = await import("../src/lib/google.js");
    (hasAntigravityStatusRuntimeAvailable as any).mockRejectedValueOnce(new Error("boom"));
