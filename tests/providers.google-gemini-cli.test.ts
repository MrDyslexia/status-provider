...
vi.mock("../src/lib/google-gemini-cli.js", () => ({
  hasGeminiCliStatusRuntimeAvailable: vi.fn(),
  queryGeminiCliStatus: vi.fn(),
}));

describe("google gemini cli provider", () => {
  it("preserves the Gemini CLI status timeout default unless requestTimeoutMs is user-configured", async () => {
    const { queryGeminiCliStatus } = await import("../src/lib/google-gemini-cli.js");
    (queryGeminiCliStatus as any).mockResolvedValue(null);

    await googleGeminiCliProvider.fetch({ client: {}, config: { requestTimeoutMs: 5000 } } as any);
    expect(queryGeminiCliStatus).toHaveBeenLastCalledWith({}, { requestTimeoutMs: undefined });

    await googleGeminiCliProvider.fetch({
      client: {},
      config: { requestTimeoutMs: 12000, requestTimeoutMsConfigured: true },
    } as any);
    expect(queryGeminiCliStatus).toHaveBeenLastCalledWith({}, { requestTimeoutMs: 12000 });
  });

  it("returns attempted:false when Gemini CLI auth is not configured", async () => {
    const { queryGeminiCliStatus } = await import("../src/lib/google-gemini-cli.js");
    (queryGeminiCliStatus as any).mockResolvedValueOnce(null);

    const out = await googleGeminiCliProvider.fetch({ client: {} } as any);
    expectNotAttempted(out);
  });

  it("maps status buckets into grouped toast entries and truncated error labels", async () => {
    const { queryGeminiCliStatus } = await import("../src/lib/google-gemini-cli.js");
    (queryGeminiCliStatus as any).mockResolvedValueOnce({
      success: true,
      buckets: [
        {
  ...
  it("is available only when the Gemini CLI runtime is configured", async () => {
    const { hasGeminiCliStatusRuntimeAvailable } = await import("../src/lib/google-gemini-cli.js");
    (hasGeminiCliStatusRuntimeAvailable as any).mockResolvedValueOnce(true);
    await expect(googleGeminiCliProvider.isAvailable({ client: {} } as any)).resolves.toBe(true);

    (hasGeminiCliStatusRuntimeAvailable as any).mockResolvedValueOnce(false);
