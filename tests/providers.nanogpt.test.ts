...
describe("nanogpt provider", () => {
  it("returns attempted:false when not configured", async () => {
    const { queryNanoGptStatus } = await import("../src/lib/nanogpt.js");
    (queryNanoGptStatus as any).mockResolvedValueOnce(null);
  ...
  it("maps canonical grouped-capable rows for daily, monthly, and balance", async () => {
    const { queryNanoGptStatus } = await import("../src/lib/nanogpt.js");
    (queryNanoGptStatus as any).mockResolvedValueOnce({
      success: true,
      subscription: {
        active: true,
        state: "active",
