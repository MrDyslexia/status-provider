...
vi.mock("../src/lib/openai.js", () => ({
  ...
  queryOpenAIStatus: vi.fn(),
}));

describe("openai provider", () => {
  it("passes configured requestTimeoutMs to the query", async () => {
    const { queryOpenAIStatus } = await import("../src/lib/openai.js");
    (queryOpenAIStatus as any).mockResolvedValueOnce(null);

    await openaiProvider.fetch({ config: { requestTimeoutMs: 12000 } } as any);

    expect(queryOpenAIStatus).toHaveBeenCalledWith({ requestTimeoutMs: 12000 });
  });
  ...
  it("maps success into canonical grouped-capable windows with single-window display metadata", async () => {
    const { queryOpenAIStatus } = await import("../src/lib/openai.js");
    (queryOpenAIStatus as any).mockResolvedValueOnce({
      success: true,
      label: "OpenAI (Pro)",
      windows: {
