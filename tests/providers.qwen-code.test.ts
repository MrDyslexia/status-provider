...
describe("qwen-code provider", () => {
  ...
  it("maps qwen free local status into canonical grouped-capable entries", async () => {
    const { readAuthFileCached } = await import("../src/lib/opencode-auth.js");
    ...
    (readQwenLocalStatusState as any).mockResolvedValue({});
    (computeQwenStatus as any).mockReturnValue({
      day: {
        used: 42,
        limit: 1000,
        percentRemaining: 96,
