...
describe("synthetic provider", () => {
  it("returns attempted:false when not configured", async () => {
    const { querySyntheticStatus } = await import("../src/lib/synthetic.js");
    (querySyntheticStatus as any).mockResolvedValueOnce(null);

    const out = await syntheticProvider.fetch({} as any);
    expectNotAttempted(out);
  });

  it("maps both Synthetic status windows into canonical grouped-capable entries with rounded usage summaries", async () => {
    const { querySyntheticStatus } = await import("../src/lib/synthetic.js");
    (querySyntheticStatus as any).mockResolvedValueOnce({
      success: true,
      windows: {
        fiveHour: {
          limit: 100,
