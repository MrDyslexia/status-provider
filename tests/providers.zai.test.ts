...
describe("zai provider", () => {
  ...
  it("returns attempted:false when not configured", async () => {
    const { queryZaiStatus } = await import("../src/lib/zai.js");
    (queryZaiStatus as any).mockResolvedValueOnce(null);
  ...
  it("maps success into canonical grouped-capable entries with single-window display metadata", async () => {
    const { queryZaiStatus } = await import("../src/lib/zai.js");
    (queryZaiStatus as any).mockResolvedValueOnce({
      success: true,
      label: "Z.ai",
      windows: {
