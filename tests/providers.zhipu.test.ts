...
describe("zhipu provider", () => {
  ...
  it("returns attempted:false when not configured", async () => {
    const { queryZhipuStatus } = await import("../src/lib/zhipu.js");
    (queryZhipuStatus as any).mockResolvedValueOnce(null);
  ...
  it("maps success into canonical grouped-capable entries with single-window display metadata", async () => {
    const { queryZhipuStatus } = await import("../src/lib/zhipu.js");
    (queryZhipuStatus as any).mockResolvedValueOnce({
      success: true,
      label: "Zhipu",
      windows: {
