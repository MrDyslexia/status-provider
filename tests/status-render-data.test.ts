...
const TEST_RUNTIME_ROOT = "/tmp/status-provider-render-data-tests";
...
} from "../src/lib/status-render-data.js";
...
describe("collectStatusRenderData shared status state", () => {
  beforeEach(async () => {
  ...
  it("reuses one canonical provider snapshot across single-window and all-window renders without mutation bleed", async () => {
    ...
    const singleWindow = await collectStatusRenderData({
      ...baseParams,
      formatStyle: "singleWindow",
    });
    ...
    const grouped = await collectStatusRenderData({
      ...baseParams,
      formatStyle: "allWindows",
    });
  ...
  it("projects Gemini quality tiers as bottleneck-only in single-window and all rows in all-windows", async () => {
    ...
    const allWindows = await collectStatusRenderData({
      ...baseParams,
      formatStyle: "allWindows",
    });
  ...
  it("keeps live-local providers uncached and returns snapshot-owned entries", async () => {
    ...
    const first = await collectStatusRenderData(params);
    ...
    const second = await collectStatusRenderData(params);
  ...
  it("collects live probes in order, projects them to single-window rows, and bypasses shared cache reuse", async () => {
    ...
    const first = await collectStatusStatusLiveProbes(params);
    const second = await collectStatusStatusLiveProbes(params);
  ...
  it("keeps legacy style ids and presentation fields working for direct render-data calls", async () => {
    ...
    const alias = await collectStatusRenderData({
      ...baseParams,
      formatStyle: "classic",
    });
    const canonical = await collectStatusRenderData({
      ...baseParams,
      formatStyle: "singleWindow",
    });
