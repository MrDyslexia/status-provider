...
import { formatStatusStatsReport } from "../src/lib/status-stats-format.js";
import type { AggregateResult } from "../src/lib/status-stats.js";
...
describe("formatStatusStatsReport (markdown)", () => {
  it("renders a markdown table for models with separator rows", () => {
    ...
    const out = formatStatusStatsReport({
  ...
  it("locks the full markdown report layout for the shared report-document renderer", () => {
    ...
    expect(body.join("\n")).toMatchInlineSnapshot(`
      ...
      Run /status-provider-info to see the full pricing diagnostics report."
    `);
  });
});
