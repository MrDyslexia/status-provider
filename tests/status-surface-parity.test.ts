...
describe("status surface parity regressions", () => {
  ...
  beforeEach(async () => {
    ...
    await resetStatusStateForTests();

    tempDir = mkdtempSync(join(tmpdir(), "status-provider-surface-parity-"));
  ...
  it("uses the same effective worktree local root for plugin and sidebar in nested-directory sessions", async () => {
    ...
    const statusOutput = getPromptText(client);
    expect(statusOutput).toContain("64% left");
  ...
  it("keeps workspace overrides for formerly global-authoritative settings aligned between plugin and sidebar", async () => {
    ...
    const statusOutput = getPromptText(client);
    expect(statusOutput).toContain("82% left");
    expect(statusOutput).not.toContain("17% left");
  ...
  it("keeps synthetic grouped numeric parity between real /status and real sidebar from shared snapshot storage", async () => {
    const syntheticProvider = {
      id: "synthetic",
      isAvailable: vi.fn().mockResolvedValue(true),
      fetch: vi.fn().mockResolvedValue({
    ...
    const statusOutput = getPromptText(client);
    expect(statusOutput).toContain("44% left");
    expect(statusOutput).toContain("8% left");
  ...
  it("keeps intentional single-window-vs-all-windows non-parity while still sharing the same underlying snapshot", async () => {
    ...
    const statusOutput = getPromptText(client);
    expect(statusOutput).toContain("95% left");
    expect(statusOutput).toContain("40% left");
