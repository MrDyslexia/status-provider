...
describe("tui panel state helpers", () => {
  ...
  it("hides the sidebar panel completely when status is disabled", () => {
    const panel: SidebarPanelState = {
      status: "disabled",
      lines: [],
    };
  ...
  it("shows a status loading compact placeholder before compact status resolves", () => {
    const panel: CompactStatusState = {
      status: "loading",
    };

    expect(shouldRenderCompactStatus(panel)).toBe(true);
    expect(getCompactStatusText(panel)).toBe("Status loading…");
  });
  ...
  it("hides compact status completely when status is disabled", () => {
    const panel: CompactStatusState = {
      status: "disabled",
      text: "Copilot 82%",
    };
