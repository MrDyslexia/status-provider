...
describe("buildSidebarStatusPanelLines", () => {
  afterEach(() => {
    vi.useRealTimers();
  });
  ...
  it("uses the fixed sidebar layout instead of toast layout settings", () => {
    ...
    const expected = formatStatusRows({
      version: "1.0.0",
  ...
  it("preserves explicit non-duration provider labels in grouped sidebar output", () => {
    ...
    expect(rendered).toContain("\nAPI ");
    expect(rendered).toContain("\nFast ");
    expect(rendered).not.toContain("Status window");
  });
  ...
  it("preserves explicit Google Antigravity labels and falls back for unlabeled legacy rows", () => {
    ...
    expect(rendered).toContain("\nClaude ");
    expect(rendered).toContain("Status window");
  ...
  it("renders grouped status windows shortest to longest in the sidebar", () => {
    const lines = buildSidebarStatusPanelLines({
      config: {
        formatStyle: "allWindows",
        percentDisplayMode: "remaining",
  ...
  it("keeps value-only rows unchanged when percentDisplayMode is used", () => {
    ...
    const remaining = buildSidebarStatusPanelLines({
      data,
      config: {
        formatStyle: "singleWindow",
        percentDisplayMode: "remaining",
    ...
    const used = buildSidebarStatusPanelLines({
      data,
      config: {
        formatStyle: "singleWindow",
        percentDisplayMode: "used",
