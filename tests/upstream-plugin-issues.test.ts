...
describe("upstream-plugin-issues", () => {
  it("builds issue bodies with the review-prep command and machine markers", () => {
    ...
    expect(body).toContain("bun run upstream:prepare-review");
    expect(body).toContain("<!-- status-provider:plugin=opencode-qwencode-auth -->");
    expect(body).toContain("<!-- status-provider:issue-state=update_available -->");
    expect(body).toContain("<!-- status-provider:latest-version=1.3.0 -->");
  });
  ...
  it("treats same-version Cursor metadata drift as an available update", () => {
    ...
    expect(plan.create?.body).toContain("<!-- status-provider:issue-state=update_available -->");
  });
